...
def solarSolve(postalCode, roofSize, usage, month, heating, budget):
    # defining parameters
    ...
    # initializing
    model = LpProblem('SolarModel', LpMinimize)
    ...
    model += ((0.35 * E[0][0]) - ((y * P * H[0] * 24 * L[0]) * (1 - d[0][0]))) == 0, 'SD0'
    model += ((0.35 * E[0][1]) - ((y * P * H[1] * 24 * L[1]) * (1 - d[0][1]))) == 0, 'SD1'
    model += ((0.35 * E[0][2]) - ((y * P * H[2] * 24 * L[2]) * (1 - d[0][2]))) == 0, 'SD2'
    model += ((0.35 * E[0][3]) - ((y * P * H[3] * 24 * L[3]) * (1 - d[0][3]))) == 0, 'SD3'
    ...
    if numPanels > 0:
        ...
        for t in range(T):
            ...
            for s in range(S):
                demandWithSolarYearly = demandWithSolarYearly + (0.65 * E[t][s]) + max(0, (0.35 * E[t][s]) - ((numPanels * P * H[s] * 24 * L[s]) * (1 - d[t][s])))
...
def batterySolve(postalCode, roofSize, usage, month, heating, storage, DoD, budget):
    # defining parameters
    E0 = usage * 3 * 1000  # seasonal electricity usage (Wh) from user
    month = month # electricity usage month from user
    heating = heating # dependent on user input electric or natural gas
...
def run_model():
    ...
    solution = solarSolve(postalCode, roofSize, usage, month, heating, budget)
...
def run_BattteryModel():
    ...
    solution = batterySolve(postalCode, roofSize, usage, month, heating, storage, DoD, budget)
    # parsing returned solution
    installationSize = solution[0]
    capitalCost = solution[1]
    paybackPeriod = solution[2]
